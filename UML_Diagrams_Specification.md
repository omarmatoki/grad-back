# مواصفات مخططات UML — منصة قياس الأثر الاجتماعي

> **ملاحظة للذكاء الاصطناعي:** هذا الملف يحتوي على وصف تفصيلي كامل لجميع مخططات UML المطلوبة للمشروع.
> اقرأ كل قسم بعناية قبل البدء في رسم أي مخطط.
> ارسم جميع المخططات بصيغة **Mermaid** داخل كتلة كود، وأضف عنواناً واضحاً وشرحاً مختصراً لكل مخطط.
> استخدم `autonumber` و `activate/deactivate` في جميع مخططات التسلسل.
...
> استخدم هذه الأسماء بالضبط كـ Lifelines في جميع مخططات التسلسل:
...
## خامساً: مواصفات مخططات حالات الاستخدام (Use Case Diagrams)

### مخطط A1: نظرة عامة على المنصة (System Overview)

**المطلوب:** مخطط شامل يضم جميع الجهات الفاعلة الخمس وجميع الوحدات الإحدى عشرة كصناديق فرعية (subgraph/subsystem)، مع أبرز 3-5 حالات استخدام لكل وحدة. أظهر علاقات التعميم بين Admin ← Manager ← Staff.
...
### مخطط A2: دورة حياة الاستبيان الكاملة (Survey Lifecycle)

**المطلوب:** مخطط تفصيلي للوحدات 7 و8 و9 يشمل جميع حالات الاستخدام (UC-31 إلى UC-44) مع كل علاقات `<<include>>` و`<<extend>>` بدقة.
...
### مخطط A3: التحليل النصي والمؤشرات (Analysis & Indicators)

**المطلوب:** مخطط تفصيلي للوحدات 10 و11 يشمل حالات الاستخدام (UC-45 إلى UC-59) مع N8N Engine كجهة فاعلة خارجية.
...
## سادساً: مواصفات مخططات التسلسل (Sequence Diagrams)

> **قاعدة عامة لجميع المخططات:**
> كل مسار محمي يبدأ دائماً بـ:
> `Client → JwtAuthGuard → MongoDB (جلب المستخدم) → RolesGuard → Controller`
> يمكن تمثيل هذا الجزء كـ `ref` fragment بعد المخطط B10.

---

### مخطط B1: تسجيل الدخول (User Login)
...
### مخطط B2: إنشاء مشروع وتعيين الفريق (Create Project & Assign Team)
...
### مخطط B3: تسجيل مستفيد في نشاط (Enroll Beneficiary in Activity)
...
### مخطط B4: بناء استبيان كامل (Create Survey with Questions & Correct Answers)
...
### مخطط B5: تقديم إجابات الاستبيان (Submit Survey Response)
...
### مخطط B6: تحليلات الاستبيان (Get Survey Analytics)
...
### مخطط B7: التحليل النصي بالذكاء الاصطناعي (AI Text Analysis + N8N Webhook)
...
### مخطط B8: تسجيل قياس مؤشر (Record Indicator Measurement)
...
### مخطط B9: حذف الاستبيان مع التسلسل (Delete Survey Cascade)
...
### مخطط B10: سلسلة حراسة المصادقة والصلاحيات (JWT Auth Guard Chain)

> **هذا مخطط مرجعي** يُستخدم في جميع المخططات الأخرى عبر `ref` fragment.
...
2. **استخدم `autonumber`** في جميع مخططات التسلسل (sequenceDiagram)
...
9. **رتّب المخططات بالترتيب:** A1 ← A2 ← A3 ← B1 ← B2 ← B3 ← B4 ← B5 ← B6 ← B7 ← B8 ← B9 ← B10
10. **لكل مخطط:** أضف عنواناً واضحاً (`## عنوان المخطط`) وشرحاً مختصراً (2-3 جمل) قبل كتلة الكود
...
*نهاية ملف المواصفات — يجب إنتاج جميع المخططات الـ 13 بالترتيب المذكور أعلاه.*
